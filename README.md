...
I used AI heavily when building this project. I started with Cursor, then switched to VS Code + Codex. This project is more or less an experiment on how fast I can build with AI without the project falling apart. I'm still making design decisions and deciding which features to build, but AI is doing most of the coding.
...
This project is just one specific way of generalizing UNO. In general, UNO can be generalized as follows: Take $k$ sets $S_1, \ldots, S_k$. Cards are a subset of $S_1 \times \ldots \times S_k \times \mathbb{N}$, where the last $\mathbb{N}$ is for duplicates of the same card. A play from $(a_1, \ldots, a_k, a)$ to $(b_1, \ldots, b_k, b)$ is allowed if and only if $(a_1, \ldots, a_k)$ and $(b_1, \ldots, b_k)$ differ in at most one coordinate.
